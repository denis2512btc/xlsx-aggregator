...
4. **Перезаписывает исходный файл** обработанной версией без дополнительного подтверждения в UI и без пользовательской резервной копии файла (см. раздел 6.8).
...
- **[Обработать]** — disabled, пока не выбран файл. По клику сразу запускает пайплайн в отдельном потоке (`threading.Thread`), чтобы окно не фризилось. Прогресс-бар обновляется из пайплайна через `queue.Queue` + `after(100)`-поллер.
- **Лог** — `CTkTextbox`, пишет события пайплайна в реальном времени.
- **По завершении** — диалог с сообщением «Готово. Файл обработан.» и кнопкой «Открыть папку» (`os.startfile(folder)` — штатно открывает Explorer на Windows).
...
MAKE_BACKUP = False         # пользовательский бэкап не создаётся
BACKUP_SUFFIX_FMT = ".backup_%Y%m%d_%H%M%S"  # зарезервировано (не используется при MAKE_BACKUP=False)
...
### 6.8. Перезапись исходника без пользовательского бэкапа (`pipeline.py`)

Перезапись оригинального файла выполняется без создания пользовательской копии `*.backup_*`.
Защита от «битого» файла обеспечивается атомарной заменой:

**1. Атомарная запись через временный файл.** Сохраняем обработанный workbook не поверх оригинала, а в соседний `.tmp`, и только после успешного `wb.save()` делаем `os.replace(tmp, original)`. Это защищает от «битого» файла при падении процесса посреди записи (электричество, kill).

**2. Проверка на lock перед стартом.** На Windows Excel держит открытый файл в эксклюзивном режиме. Пробуем открыть `original` на запись в начале — при `PermissionError` показываем диалог «Закройте файл в Excel и повторите» и не трогаем ничего.

**3. Активный лист при открытии результата.** Перед сохранением выставляем активный лист книги:
- `YQ2PF`, если режим обработки `YQ`;
- `YW2PF`, если режим `YW`.
...
def safe_overwrite_save(wb, original_path: str) -> dict:
    """Сохраняет wb поверх original_path атомарной заменой.
    Возвращает dict с путями: {'result': ...}."""
    ...
    # 2) Запись в .tmp в той же директории (важно для os.replace на одной FS)
    ...
    try:
        ...
        # откат: временный файл удаляем, оригинал не тронут
        if tmp_path.exists():
            tmp_path.unlink()
        raise
...
- Диалог «Готово. Файл обработан.» + кнопка «Открыть папку».
...
| **Файл уже обрабатывался** | Если на целевом листе (`YW2PF`/`YQ2PF`) есть непустые ячейки в строках с 4-й и ниже — пайплайн останавливается с сообщением «Файл уже обрабатывался». |
| **Исходный файл открыт в Excel** | `PermissionError` ловим **до любых операций** (см. 6.8) → диалог «Файл занят — закройте его в Excel». |
| Ошибка посреди записи | Временный `.tmp` удаляется. Оригинал не тронут (т.к. `os.replace` ещё не отработал). |
| Любая другая ошибка | Полный traceback → `%LOCALAPPDATA%\xlsx_aggregator\logs\error.log`, в UI — короткое сообщение + кнопка «Открыть лог». |
...
| 3 | **Перезапись исходника без пользовательского бэкапа**. Перед сохранением применяется атомарная запись через `.tmp` + `os.replace` (см. 6.8). |
| 4 | **Проверка «уже обрабатывался».** Если на целевом листе (`YW2PF`/`YQ2PF`) есть непустые данные с 4-й строки и ниже, запуск прекращается с сообщением «Файл уже обрабатывался». |
...
| 11 | **Активный лист результата:** после обработки файл открывается с листа `YQ2PF` (YQ-режим) или `YW2PF` (YW-режим). |
...
6. **Язык UI** — русский.
7. **Порог «уже обрабатывался».** Для `YW2PF`/`YQ2PF` исходными считаются только строки 1–3; непустые ячейки со строки 4 и ниже означают, что файл уже обрабатывался, и запуск прерывается.
...
| 7 | `writer.py` — запись блоков с маркерами `[XA:*]`, формулой и автофильтром + `_strip_previous_run` | `src/core/writer.py` | Результат пишется на целевой лист по текущим правилам формирования блоков |
| 8 | `pipeline.py` — оркестрация + `safe_overwrite_save` (lock-check + атомарная запись через `.tmp`) + колбэк прогресса (%) + активный лист | `src/core/pipeline.py` | Из CLI (`python -m src.core.pipeline file.xlsx`) работает без GUI; при открытом в Excel файле даёт понятную ошибку |
| 9 | `gui/app.py` — customtkinter, 2 кнопки, прогресс, лог-вьюшка, поток, запуск без диалога подтверждения перезаписи | `src/gui/app.py` | GUI запускает pipeline без фриза окна; по завершении показывает успех без упоминания бэкапа |
...
| 21 | Проверка «файл уже обрабатывался» (строки 4+) + запрет повторной обработки + открытие результата с целевого листа | `src/core/pipeline.py`, `src/gui/app.py`, `tests/test_pipeline_yq.py`, `tests/test_pipeline.py` | При данных ниже 3-й строки на `YW2PF/YQ2PF` показывается ошибка; после успешной обработки активен нужный лист |
...
- [ ] Если на `YW2PF`/`YQ2PF` есть непустые данные со строки 4 и ниже, приложение показывает «Файл уже обрабатывался» и прекращает обработку.
- [ ] Если файл открыт в Excel — приложение выдаёт понятную ошибку и НЕ портит файл.
...
- [ ] После успешной обработки активным листом книги является `YW2PF` (YW-режим) или `YQ2PF` (YQ-режим).
